...
public class SeccionDB {
    private static final String metodoWeb = "BBWS06ObtenerSeccion"; //BBWS7ObtenerMeta?PIN=&IMEI=&IMSI=&IDAPP=&FFVV=FS&CodigoPais=21&NombreUsuario=1215&GMT=&Rol=
    private static String URL, DATA;
    private static PersistentObject persist;
    private static final long IDSTORE = 0x669c45f4bb8316e4L; // com.belcorp.entidades.Seccion    
    private static Vector objetos;
    private Usuario usuario;

    /**
     * Constructor del DAO para las seccciones de la gerente
     */
    public SeccionDB() {
     ...
     * @param node
     * @return si se puede formatear la información del XML a los objetos respectivos de las secciones de la gerente
     * @throws Exception
     */
    private boolean fillObjectos(NodeList node) throws Exception {
        ...
        for (int i = 1; i < n; i = i + 2) {
            ...
            seccion.setId(fields[0]);
            seccion.setDescripcion(fields[0]);
            objetos.addElement(seccion);
        }
        persist.setContents(objetos);
        persist.commit();
        return true;
    ...
    /**
     * Metodo que devuelve un objeto de seccion en base a un filtro de codigo de seccion
     * @param idSecccion valor del codigo de seccion a buscar
     * @return Seccion objeto encontrado en base al filtro
     */
    public Seccion getSeccionById(String idSecccion) {
        Seccion seccion = null;
        int i, n;
        n = objetos.size();
        for (i = 0; i < n; i++) {
            seccion = (Seccion) objetos.elementAt(i);
            if(idSecccion.equals(seccion.getId())){
                return seccion;
            }
        }
        return seccion;
    ...
    /**
     * Metodo que retorna el codigo de la seccion en base a una busqueda por descripcion
     * @param sDescrip valor de la descripcion a buscar
     * @return String codigo de la seccion encontrada
     */
    public String getSeccionByNombre(String sDescrip) {
        Seccion seccion = null;
        int i, n;
        n = objetos.size();
        for (i = 0; i < n; i++) {
            seccion = (Seccion) objetos.elementAt(i);
            if(sDescrip.equals(seccion.getDescripcion())){
